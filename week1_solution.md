...
### 회고
```
매일 아침, 가벼운 한 줄 멘트로 분위기를 풀기 시작했다.
처음엔 반응이 조용했지만, 점점 기다리는 눈치가 느껴졌다.
멘트 하나로 대화의 물꼬가 트이고, 멋쩍지만 웃음이 생겼다.
나름대로 어색함을 깨뜨리는 데 꽤 효과적이었다는 생각이 들었다.
```
---
...
릴레이 퀘스트를 통해 전달받은 프롬프트는 충분한 고민 없이 코드만을 요구하거나 상황에 대한 설명 없이 반복적인 질문을 할 경우 등 학습에 적절하지 않게 AI를 사용하는 상황에 대해서 경고하는 트리거가 있었습니다. 이 덕분에 함수형 프로그래밍으로 Linked List를 구현하는 미션에서 스스로 생각하는 힘을 기를 수 있었습니다. AI가 `append` 함수에 대해서 case별로 빈칸을 뚫어서 저에게 줬는데 빈칸에 어떤 로직이 들어갈지 계속해서 머리를 쥐어짜다 보니 불변성과 재귀 함수를 통한 함수형 프로그래밍 패러다임에 점점 눈이 떠지는 것을 느낄 수 있었습니다. 이를 통해 올바른 AI 사용법을 배울 수 있었고 저의 AI 사용 습관이나 학습 습관 또한 점검받을 수 있었기에 좋은 릴레이 퀘스트였다고 생각합니다.
